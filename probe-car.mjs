...
await p.goto(`http://localhost:4173/?peds=0&traffic=0&shadows=0&intro=0&nohud=1&car=${kind}${kind === 'police' ? '&stars=1' : ''}`);
...
const car = await p.evaluate((kind) => {
  let v = window.__session.vehicles[0];
  if (kind === 'police') {
    v = window.__session.police.cars.find((c) => c.group.visible) ?? v;
    const me = window.__session.player.pos;
    v.reset(me.x + 8, me.z, 0);
  }
